...
class ApiClient extends GetxService {
  ...
  Uri _buildUri(String uri) {
    final String base = appBaseUrl.replaceFirst(RegExp(r'/+$'), '');
    final String path = uri.replaceFirst(RegExp(r'^/+'), '');
  ...
  Future<Response> postMultipartData(
    ...
    try {
      ...
      http.MultipartRequest request =
          http.MultipartRequest('POST', _buildUri(uri));
      ...
      if (otherFile.isNotEmpty) {
        for (MultipartDocument file in otherFile) {
          ...
          final aaa = File(platformFile.path!);
          final list0 = await aaa.readAsBytes();
