...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> register(String code, SignUpBody signUpBody) async {
    ...
    if (response!.statusCode == 200) {
      ...
      // Prevent "nothing happens" when backend returns validation errors (400/422/etc)
      final String? bodyString = response.bodyString;
      if (bodyString != null && bodyString.isNotEmpty) {
        showCustomSnackBar(
          'registration_failed'.tr + ' (${response.statusCode}): $bodyString',
          isError: true,
        );
      } else {
        showCustomSnackBar(
          'registration_failed'.tr + ' (${response.statusCode})',
          isError: true,
        );
      }
      ApiChecker.checkApi(response);
  ...
  Future<Response?> otpVerification(String phoneNumber, String otp,
    ...
    if (response?.statusCode == 200) {
      ...
      if (from == VerificationForm.signUp) {
        ...
        String? deviceToken =
            kLocalDev ? null : await FirebaseMessaging.instance.getToken();
        _isLoading = false;
        registerWithOtp(
            SignUpBody(
