...
class FaceVerificationController extends GetxController implements GetxService {
  ...
  Future startLiveFeed() async {
    if (GetPlatform.isWeb) {
      showCustomSnackBar('Camera is not supported on web.');
      return;
    }
    ...
    if (cameras.isEmpty) {
      try {
        ...
        showCustomSnackBar('Camera is not available on this device.');
        return;
      }
    }
    ...
    controller?.initialize().then((_) {
      ...
      controller?.startImageStream((CameraImage cameraImage) =>
          _inputImageFromCameraImage(image: cameraImage, camera: camera));
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    try {
      ...
      if (faces.length == 1) {
        if ((faces[0].rightEyeOpenProbability ?? 1) < 0.1 &&
            (faces[0].leftEyeOpenProbability ?? 1) < 0.1 &&
            _eyeBlink < 3) {
          _eyeBlink++;
        }
      }
  ...
  Future<void> processPicture(InputImage inputImage) async {
    ...
    if (hasEyeOpen || GetPlatform.isIOS) {
      Future.delayed(const Duration(seconds: 1)).then((value) async {
        ...
        Get.dialog(const FaceVerifyingDialog(), barrierDismissible: false);

        Response response = await faceVerificationServiceInterface
            .verifyDriverIdentity(compressXFile);
        stopLiveFeed();
  ...
  Future<XFile> compressFile(File file) async {
    ...
    XFile? result = await FlutterImageCompress.compressAndGetFile(
      file.absolute.path,
      _generateCompressedFilePath(filePath),
      quality: 50,
    );
  ...
  void skipFaceVerification({bool fromVarificationScreen = false}) async {
    Response response =
        await faceVerificationServiceInterface.skipVerification();
