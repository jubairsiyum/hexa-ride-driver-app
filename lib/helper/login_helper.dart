...
import 'package:ride_sharing_user_app/features/subscription/screens/subscription_required_screen.dart';
...
class LoginHelper {
          ...
          Get.find<ProfileController>().getProfileInfo().then((value) {
            if (value.statusCode == 200) {
              Get.find<SubscriptionController>()
                  .getCurrentSubscription(notify: false)
                  .then((_) {
                if (!Get.find<SubscriptionController>().hasActiveSubscription) {
                  Get.offAll(() => const SubscriptionRequiredScreen());
                  return;
                }
  ...
  static void checkLoginMedium() {
    final bool isManualLogin =
        Get.find<SplashController>().config?.driverLoginOptions?.manualLogin ??
            false;
    if (isManualLogin) {
